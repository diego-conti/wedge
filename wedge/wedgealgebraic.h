...
namespace internal {
//code used in initialization to obtain the class name. %GiNaC classes and %Wedge Classes must
//be handled differently, since the first do not have a static_class_name() member, whereas the 
//class_info static member of the latter might not be initialized yet.
template<typename T> struct GiveClassName {
	struct I : public T {		
		static const char* GetClassName() {return static_class_name();}
	};
	static const char* static_class_name()
	{
		//This code is called during static initialization of a data member of a class template,
		//and it relies on the static members of the GiNaC class T having been initialized first.
		//In principle, the C++ standard does not guarantee that this will work, but since
		//the GiNaC objects are initialized in libginac, and libwedge depends explicitly on libginac,
		//it works in practice (at least with the gcc linker).    
		return T::get_class_info_static().options.get_name();
	}
...
template<typename subclass,typename superclass> 
  GiNaC::registered_class_info Registered<subclass,superclass,true,true>::reg_info =
	GiNaC::registered_class_info(GiNaC::registered_class_options(
		...
		internal::GiveClassName<superclass>::GetClassName(),
		typeid(subclass)
	));
template<typename subclass,typename superclass> 
