...
*/
	...
	matrix Simplify(const matrix& m) const {
		...
		auto simplified=Simplify(as_vector);
		return matrix{m.rows(),m.cols(),lst{simplified.begin(),simplified.end()}};
	}

};

struct RootsSimplifier: public Simplifier {
	ex SimplifyScalar(ex e) const override;
};

extern RootsSimplifier roots_simplifier;
